...
function CompanyLogo() {
  ...
  return (
    <div className='bg-black overflow-hidden md:py-16  py-8 text-white relative'>
                ...
                [logos, logos].map((logoSet, setIndex)=>(
                    <div key={setIndex} className='flex shrink-0 min-w-full items-center justify-around px-4'> 
                    {
                       logoSet.map((logo, index) => (
                        <img key={`${setIndex}-${index}`} src={logo.src} alt={logo.alt} width={logo.width} height={logo.height} className='mx-8 object-contain shrink-0'/>
                    ))
                    }
        ...
        {
            logos.map((logo, idx)=>(
                <img src={logo.src} alt={logo.alt} width={logo.width} height={logo.height} className='mx-8 object-contain shrink-0'/>
            ))
        }
        </div>
